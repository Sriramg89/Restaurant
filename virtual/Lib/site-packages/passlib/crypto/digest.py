...
def _get_hash_aliases(name):
    ...
    m = re.match(
        r"(?i)^(?P<name>[a-z]+)-?(?P<rev>\d)?-?(?P<size>\d{3,4})?$", name)
    if m:
...
def lookup_hash(digest,  # *,
    ...
    if isinstance(digest, unicode_or_bytes_types):
        ...
        raise exc.ExpectedTypeError(
            digest, "digest name or constructor", "digest")
    ...
    if cache_by_name:
        for name in name_list:
            if name:  # (skips iana name if it's empty)
                assert cache.get(name) in [
                    None, info], "%r already in cache" % name
                cache[name] = info
    return info
...
class HashInfo(SequenceMixin):
    ...
    def __init__(self,  # *,
        ...
        if len(hash.digest()) != hash.digest_size:
            raise RuntimeError(
                "%r constructor failed sanity check" % self.name)
    ...
    def _get_pbkdf2_looper(digest_size):
        ...
        if _have_64_bit and not digest_size & 0x7:
            ...
            raise NotImplementedError(
                "unsupported digest size: %d" % digest_size)
        struct = Struct(fmt)
