...
class AppWallet(object):
    ...
    def __init__(self, secrets=None, default_tag=None, encrypt_cost=None,
        ...
        if secrets_path is not None:
            if secrets is not None:
                raise TypeError(
                    "'secrets' and 'secrets_path' are mutually exclusive")
    ...
    def _cipher_aes_key(value, secret, salt, cost, decrypt=False):
        ...
        keyiv = pbkdf2_hmac("sha256", secret, salt=salt,
                            rounds=(1 << cost), keylen=48)
    ...
    def encrypt_key(self, key):
        ...
        if not tag:
            raise TypeError(
                "no application secrets configured, can't encrypt OTP key")
    ...
    def decrypt_key(self, enckey):
        ...
        if version == 1:
            ...
            raise ValueError(
                "missing / unrecognized 'enckey' version: %r" % (version,))
        tag = enckey['t']
        cost = enckey['c']
        key = _cipher_key(
...
class TOTP(object):
    ...
    def __init__(self, key=None, format="base32",
        ...
        if not isinstance(digits, int_types):
            raise TypeError(
                "digits must be an integer, not a %r" % type(digits))
    ...
    def encrypted_key(self, value):
        ...
        if not wallet:
            raise TypeError(
                "no application secrets present, can't decrypt TOTP key")
    ...
    def normalize_time(cls, time):
        ...
        if isinstance(time, int_types):
            ...
            raise exc.ExpectedTypeError(
                time, "int, float, or datetime", "time")
        ...
        if isinstance(token, int_types):
            ...
            if not token.isdigit():
                raise MalformedTokenError(
                    "Token must contain only the digits 0-9")
        if len(token) != digits:
            raise MalformedTokenError(
                "Token must have exactly %d digits" % digits)
        return token
...
#            counter += 1
    ...
    def to_uri(self, label=None, issuer=None):
        ...
        param_str = u("&").join(u("%s=%s") % (key, quote(value, ''))
    ...
    def _adapt_dict_kwds(cls, type, **kwds):
        ...
        if not ver or ver < cls.min_json_version or ver > cls.json_version:
            raise cls._dict_parse_error(
                "missing/unsupported version (%r)" % (ver,))
