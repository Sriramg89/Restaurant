...
#    relaxed = False # when _norm_xxx() funcs should be strict about inputs
    ...
    def _norm_checksum(self, checksum, relaxed=False):
        ...
        if not raw:
            ...
            if cs and any(c not in cs for c in checksum):
                raise ValueError(
                    "invalid characters in %s checksum" % (self.name,))
    ...
    def to_string(self):  # pragma: no cover
        ...
        raise NotImplementedError(
            "%s must implement from_string()" % (self.__class__,))
    ...
    def using(cls,  # keyword only...
        ...
        if ident is not None:
            if default_ident is not None:
                raise TypeError(
                    "'default_ident' and 'ident' are mutually exclusive")
            default_ident = ident
        ...
        if default_ident is not None:
            subcls.default_ident = cls(
                ident=default_ident, use_defaults=True).ident
        return subcls
...
class HasRounds(GenericHandler):
    ...
    def __init__(self, rounds=None, **kwds):
        ...
        if rounds is not None:
            ...
            assert self._norm_rounds(
                rounds) == rounds, "generated invalid rounds: %r" % (rounds,)
        else:
    ...
    *not* the overall class.
    ...
    def _get_backend_owner(cls):
        ...
        raise AssertionError(
            "expected to find class w/ '_backend_mixin_target' set")
    ...
    def _set_backend(cls, name, dryrun):
        ...
        assert issubclass(
            mixin_cls, SubclassBackendMixin), "invalid mixin class"
