    ...
    if source_path:
        if not os.path.exists(source_path):
            raise EnvironmentError(
                "django source path not found: %r" % source_path)
        if not all(os.path.exists(os.path.join(source_path, name))
                   for name in ["django", "tests"]):
            raise EnvironmentError(
                "invalid django source path: %r" % source_path)
...
if test_hashers_mod:
    ...
    class HashersTest(test_hashers_mod.TestUtilsHashPass, _ExtensionSupport):
        ...
        def setUp(self):
            ...
            self.patchAttr(adapter, "django_to_passlib",
                           update_wrapper, wrap=True)
        ...
        def _OMIT(self): return self.skipTest("omitted by passlib")
