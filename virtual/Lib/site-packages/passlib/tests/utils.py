...
def handler_derived_from(handler, base):
    ...
    if handler == base:
        ...
        raise NotImplementedError(
            "don't know how to inspect handler: %r" % (handler,))
    ...
    def setUpWarnings(self):
        ...
        if self.resetWarningState:
            ...
            warnings.filterwarnings(
                "ignore", r"the method .*\.(encrypt|genconfig|genhash)\(\) is deprecated")
            warnings.filterwarnings(
                "ignore", r"the 'vary_rounds' option is deprecated")
            warnings.filterwarnings(
    ...
    def subTest(self, *args, **kwds):
        ...
        with ctx:
            ...
            try:
                ...
                test_log.warning("subtest failed: %s: %s: %r",
                                 title, type(err).__name__, str(err))
                raise
...
class HandlerCase(TestCase):
    ...
    def create_backend_case(cls, backend):
        ...
        name = handler.name
        assert hasattr(
            handler, "backends"), "handler must support uh.HasManyBackends protocol"
    ...
    def test_02_config_workflow(self):
        ...
        # usually true, but not required by protocol
            ...
            for c in cls.default_salt_chars:
                if c not in cls.salt_chars:
                    raise AssertionError(
                        "default_salt_chars must be subset of salt_chars: %r not in salt_chars" % (c,))
        else:
            if not cls.default_salt_chars:
                raise AssertionError(
    ...
    def salt_bits(self):
        ...
        handler = self.handler
        assert has_salt_info(
            handler), "need explicit bit-size for " + handler.name
        from math import log
    ...
    def test_13_max_salt_size(self):
        ...
        if max_size is None or max_size > (1 << 20):
            ...
            self.assertRaises(ValueError, self.do_stub_encrypt,
    ...
    def test_15_salt_type(self):
        ...
        if salt_type is not unicode:
            self.assertRaises(TypeError, self.do_encrypt,
                              'stub', salt=u('x') * salt_size)
    ...
    def test_using_salt_size(self):
        ...
        if mx:
            self.assertRaises(ValueError, handler.using,
                              default_salt_size=mx+1)
            with self.assertWarningList([PasslibHashWarning]):
    ...
    def test_has_rounds_using_w_min_rounds(self):
        ...
        if orig_min_rounds > 0:
            self.assertRaises(ValueError, handler.using,
                              min_desired_rounds=orig_min_rounds - adj)
            with self.assertWarningList([PasslibHashWarning]):
                temp = handler.using(
        ...
        if orig_max_rounds:
            self.assertRaises(ValueError, handler.using,
                              min_desired_rounds=orig_max_rounds + adj)
            with self.assertWarningList([PasslibHashWarning]):
                temp = handler.using(
        ...
        self.assertEqual(get_effective_rounds(
            subcls, small + adj), small + adj)
    ...
    def test_has_rounds_replace_w_max_rounds(self):
        ...
        if orig_min_rounds > 0:
            self.assertRaises(ValueError, handler.using,
                              max_desired_rounds=orig_min_rounds - adj)
            with self.assertWarningList([PasslibHashWarning]):
                temp = handler.using(
        ...
        if orig_max_rounds:
            self.assertRaises(ValueError, handler.using,
                              max_desired_rounds=orig_max_rounds + adj)
            with self.assertWarningList([PasslibHashWarning]):
                temp = handler.using(
        ...
        # invalid strings should cause error
        self.assertRaises(ValueError, handler.using,
                          max_desired_rounds=str(large) + "xxx")
    ...
    def test_has_rounds_using_w_vary_rounds_generation(self):
        ...
        def assert_rounds_range(vary_rounds, lower, upper):
            ...
            self.assertEqual(seen_lower, lower,
                             "vary_rounds had wrong lower limit:")
            self.assertEqual(seen_upper, upper,
        ...
        assert_rounds_range(50, max(small, medium - 50),
                            min(large, medium + 50))
    ...
    def test_has_many_idents_using(self):
        ...
        self.assertRaises(TypeError, handler.using,
                          default_ident=alt_ident, ident=alt_ident)
        ...
        if handler.ident_aliases:
            for alias, ident in handler.ident_aliases.items():
                ...
                self.assertEqual(subcls.default_ident, ident,
                                 msg="alias %r:" % alias)
    ...
    def test_63_large_secret(self):
        ...
        err = self.assertRaises(
            PasswordSizeError, self.do_genhash, secret, hash)
    ...
    def test_77_fuzz_input(self, threaded=False):
        ...
        * runs output against other "external" verifiers such as OS crypt()
        ...
        log.debug("%s: %s: started; max_time=%r verifiers=%d (%s)",
                  self.descriptionPrefix, thread_name, max_time, len(
                      verifiers),
...
class OsCryptMixin(HandlerCase):
    ...
    * check that native crypt support is detected correctly for known platforms.
    ...
    def _patch_safe_crypt(self):
        ...
        if not alt_backend:
            raise AssertionError(
                "handler has no available alternate backends!")
...
class reset_warnings(warnings.catch_warnings):
    ...
    def __init__(self, reset_filter="always", reset_registry=".*", **kwds):
        ...
        self._reset_registry = re.compile(
            reset_registry) if reset_registry else None
