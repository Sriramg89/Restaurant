...
class _ExtensionSupport(object):
    ...
    def assert_unpatched(self):
        ...
        self.assertFalse(mod and mod.adapter.patched,
                         "patch should not be enabled")
    ...
    _config_keys = ["PASSLIB_CONFIG",
                    "PASSLIB_CONTEXT", "PASSLIB_GET_CATEGORY"]
...
class DjangoBehaviorTest(_ExtensionTest):
    ...
    def test_extension_config(self):
        ...
        self.assertEqual(password_context.to_dict(
            resolve=True), ctx.to_dict(resolve=True))
    ...
    def _do_test_invalid_hash_value(self, hash):
        ...
        # User.has_usable_password()
        self.assertEqual(user.has_usable_password(),
                         quirks.invalid_is_usable_password)
    ...
    def _do_test_available_scheme(self, scheme):
        ...
        if not patched and not check_django_hasher_has_backend(handler.django_name):
            ...
            log.warning(
                "skipping scheme %r due to missing django dependency", scheme)
