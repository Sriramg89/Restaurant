...
"""
    ...
    def test_11_load_rollback(self):
        ...
        self.assertRaises(ValueError, cc.update,
                          sha256_crypt__min_rounds=10000)
        self.assertEqual(cc.to_string(), result)
    ...
    def test_20_options(self):
        ...
        self.assertEqual(
            parse(**{"default.context.default": "md5_crypt"}), result)
        ...
        self.assertEqual(
            parse(**{"admin.context.default": "md5_crypt"}), result)
...
##        self.assertEqual(getdep(cc), ["md5_crypt"])
        ...
        # comma sep list
        cc = CryptContext(deprecated="md5_crypt,des_crypt", schemes=[
                          "md5_crypt", "des_crypt", "sha256_crypt"])
    ...
    def test_23_default(self):
        ...
        ctx = CryptContext(default="md5_crypt", schemes=[
                           "des_crypt", "md5_crypt"])
        ...
        ctx = CryptContext(default=hash.md5_crypt, schemes=[
                           "des_crypt", "md5_crypt"])
        self.assertEqual(ctx.default_scheme(), "md5_crypt")
    ...
    def test_30_schemes(self):
        ...
        self.assertEqual(ctx.schemes(
            resolve=True, unconfigured=True), tuple(self.sample_1_handlers))
    ...
    def test_31_default_scheme(self):
        ...
        ctx = CryptContext(**self.sample_1_dict)
        self.assertEqual(ctx.default_scheme(), "md5_crypt")
        self.assertEqual(ctx.default_scheme(
            resolve=True, unconfigured=True), hash.md5_crypt)
        self.assertHandlerDerivedFrom(
            ctx.default_scheme(resolve=True), hash.md5_crypt)
    ...
    def test_32_handler(self):
        ...
        # by name
        self.assertEqual(ctx.handler(
            "des_crypt", unconfigured=True), hash.des_crypt)
        ...
        ctx = CryptContext("sha256_crypt,md5_crypt",
                           admin__context__default="md5_crypt")
        ...
        self.assertEqual(ctx.handler(category="staff",
                                     unconfigured=True), hash.sha256_crypt)
        self.assertHandlerDerivedFrom(ctx.handler(
            category="staff"), hash.sha256_crypt)

        self.assertEqual(ctx.handler(category="admin",
                                     unconfigured=True), hash.md5_crypt)
        self.assertHandlerDerivedFrom(ctx.handler(
            category="staff"), hash.sha256_crypt)

        # test unicode category strings are accepted under py2
        if PY2:
            self.assertEqual(ctx.handler(category=u("staff"),
                                         unconfigured=True), hash.sha256_crypt)
            self.assertEqual(ctx.handler(category=u("admin"),
                                         unconfigured=True), hash.md5_crypt)
    ...
    def test_34_to_dict(self):
        ...
        ctx = CryptContext(**self.sample_1_dict)
        self.assertEqual(ctx.to_dict(), self.sample_1_dict)
        self.assertEqual(ctx.to_dict(resolve=True),
                         self.sample_1_resolved_dict)
    ...
    def test_35_to_string(self):
        ...
        self.assertEqual(other, dump.replace(
    ...
    def test_40_basic(self):
        ...
        for crypt in handlers:
            ...
            self.assertEqual(cc.identify(
                h, resolve=True, unconfigured=True), crypt)
        ...
        self.assertRaises(ValueError, cc.genhash, 'secret',
                          cc.genconfig(), scheme="des_crypt")
    ...
    def test_41_genconfig(self):
        ...
        # category override
        self.assertTrue(cc.genconfig(scheme="phpass",
                                     category="admin").startswith("$P$5"))
        self.assertTrue(cc.genconfig(scheme="phpass",
                                     category="staff").startswith("$H$5"))
        ...
        if PY2:
            ...
            self.assertTrue(c2.genconfig(
                category=u("admin")).startswith("$P$5"))
            self.assertTrue(c2.genconfig(
                category=u("staff")).startswith("$H$5"))
    ...
    def test_44_identify(self):
        ...
        self.assertRaises(ValueError, cc.identify,
                          '$9$232323123$1287319827', required=True)
    ...
    def test_45_verify(self):
        ...
        self.assertRaises(ValueError, cc.verify, 'test',
                          h, scheme='bsdi_crypt')
        ...
        self.assertRaises(ValueError, cc.verify, 'stub',
                          '$6$232323123$1287319827')
        ...
        # bad scheme values
        # XXX: should this be ValueError?
        self.assertRaises(KeyError, cc.verify, 'secret',
                          refhash, scheme="fake")
    ...
    def test_46_needs_update(self):
        ...
        # bad scheme values
        self.assertRaises(KeyError, cc.needs_update, refhash,
                          scheme="fake")  # XXX: should this be ValueError?
    ...
    def test_47_verify_and_update(self):
        ...
        for secret, kwds in self.nonstring_vectors:
            self.assertRaises(TypeError, cc.verify_and_update,
                              secret, hash, **kwds)
        ...
        # throws error without schemes
        self.assertRaises(KeyError, CryptContext(
        ).verify_and_update, 'secret', 'hash')

        # bad scheme values
        self.assertRaises(KeyError, cc.verify_and_update, 'secret',
                          refhash, scheme="fake")  # XXX: should this be ValueError?
        self.assertRaises(TypeError, cc.verify_and_update,
                          'secret', refhash, scheme=1)

        # bad category values
        self.assertRaises(TypeError, cc.verify_and_update,
                          'secret', refhash, category=1)
    ...
    def test_48_context_kwds(self):
        ...
        self.assertRaises(TypeError, cc1.verify_and_update,
                          "stub", des_hash, user="root")
        ...
        self.assertTrue(des_crypt.identify(
            cc2.hash("stub", user="root")), "des_crypt")
        self.assertTrue(cc2.verify("stub", des_hash, user="root"))
        self.assertEqual(cc2.verify_and_update(
            "stub", des_hash, user="root"), (True, None))

        # verify error with unknown kwd
        with self.assertWarningList(["passing settings to.*is deprecated"]):
            self.assertRaises(TypeError, cc2.hash, "stub", badkwd="root")
        self.assertRaises(TypeError, cc2.verify, "stub",
                          des_hash, badkwd="root")
        self.assertRaises(TypeError, cc2.verify_and_update,
                          "stub", des_hash, badkwd="root")
        ...
        self.assertRaises(TypeError, cc3.verify_and_update,
                          "stub", pg_root_hash)
        ...
        self.assertEqual(cc3.verify_and_update(
            "stub", pg_root_hash, user="root"), (True, None))

        # verify_and_update() should fail against wrong user
        self.assertEqual(cc3.verify_and_update(
            "stub", pg_root_hash, user="admin"), (False, None))
    ...
    def test_50_rounds_limits(self):
        ...
        # test bad values
        self.assertRaises(ValueError, CryptContext,
                          sha256_crypt__min_rounds='x')
        self.assertRaises(ValueError, CryptContext,
                          sha256_crypt__max_rounds='x')
        self.assertRaises(ValueError, CryptContext, all__vary_rounds='x')
        self.assertRaises(ValueError, CryptContext,
                          sha256_crypt__default_rounds='x')

        # test bad types rejected
        bad = datetime.datetime.now()  # picked cause can't be compared to int
        self.assertRaises(TypeError, CryptContext,
                          "sha256_crypt", sha256_crypt__min_rounds=bad)
        self.assertRaises(TypeError, CryptContext,
                          "sha256_crypt", sha256_crypt__max_rounds=bad)
        self.assertRaises(TypeError, CryptContext,
                          "sha256_crypt", all__vary_rounds=bad)
        self.assertRaises(TypeError, CryptContext,
                          "sha256_crypt", sha256_crypt__default_rounds=bad)
    ...
    def test_51_linear_vary_rounds(self):
        ...
        self.assertEqual(c2._get_record(
            "sha256_crypt", None).vary_rounds, 0.001)
    ...
    def assert_rounds_range(self, context, scheme, lower, upper):
        ...
        for i in irange(300):
            ...
            seen.add(r)
        self.assertEqual(min(seen), lower,
                         "vary_rounds had wrong lower limit:")
        self.assertEqual(max(seen), upper,
                         "vary_rounds had wrong upper limit:")
    ...
    def test_61_autodeprecate(self):
        ...
        ctx = CryptContext(
            "sha256_crypt,md5_crypt,des_crypt", deprecated="auto")
...
class LazyCryptContextTest(TestCase):
    ...
    def test_kwd_constructor(self):
        ...
        cc = LazyCryptContext(
            iter(["dummy_2", "des_crypt"]), deprecated=["des_crypt"])
