...
class _BcryptCommon(uh.SubclassBackendMixin, uh.TruncateMixin, uh.HasManyIdents,
    ...
    def to_string(self):
        hash = u("%s%02d$%s%s") % (
            self.ident, self.rounds, self.salt, self.checksum)
    ...
    def _norm_checksum(self, checksum, relaxed=False):
        checksum = super(_BcryptCommon, self)._norm_checksum(
            checksum, relaxed=relaxed)
    ...
    def _finalize_backend_mixin(mixin_cls, backend, dryrun):
        ...
        def assert_lacks_8bit_bug(ident):
            ...
            bug_hash = ident.encode(
                "ascii") + b"05$6bNw2HLQYeqHYyBfLMsv/OiwqTymGIGzFsA4hOTWebfehXHNprcAS"
            correct_hash = ident.encode(
                "ascii") + b"05$6bNw2HLQYeqHYyBfLMsv/OUcZd0LKP39b87nBw3.S2tVZSqiQX6eu"
        ...
        def detect_wrap_bug(ident):
            ...
            bug_hash = ident.encode(
                "ascii") + b"04$R1lJ2gkNaoPGdafE.H.16.nVyh2niHsGJhayOHLMiXlI45o8/DU.6"
            if verify(secret, bug_hash):
                return True
            ...
            correct_hash = ident.encode(
                "ascii") + b"04$R1lJ2gkNaoPGdafE.H.16.1MKHPvmKwryeulRe225LKProWYwt9Oi"
            if not verify(secret, correct_hash):
                raise RuntimeError(
        ...
        if result is NotImplemented:
            ...
            log.debug(
                "%r backend lacks $2$ support, enabling workaround", backend)
        elif not result:
        ...
        if result is NotImplemented:
            ...
            if detect_wrap_bug(IDENT_2A):
                if backend == "os_crypt":
                    ...
                    log.debug(
                        "%r backend has $2a$ bsd wraparound bug, enabling workaround", backend)
                else:
        ...
        if result is NotImplemented:
            ...
            log.debug(
                "%r backend lacks $2y$ support, enabling workaround", backend)
        elif not result:
        ...
        if result is NotImplemented:
            ...
            log.debug(
                "%r backend lacks $2b$ support, enabling workaround", backend)
        elif not result:
...
class bcrypt_sha256(_wrapped_bcrypt):
    ...
    def to_string(self):
        ...
        hash = template % (self.ident.strip(_UDOLLAR),
